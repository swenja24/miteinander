...
function tasksPage(){
  ...
  const matchesDue=t=>taskFilters.due==='all'||(taskFilters.due==='overdue'&&t.due&&t.due<today()&&!['done','refused'].includes(t.status))||(taskFilters.due==='today'&&t.due===today())||(taskFilters.due==='week'&&t.due>=today()&&t.due<=nextWeekValue)||(taskFilters.due==='none'&&!t.due);
  ...
  const filters=`<div class="card task-filters"><label>Kategorie<select data-task-filter="category"><option value="all">Alle Kategorien</option>${categories.map(value=>`<option ${taskFilters.category===value?'selected':''}>${esc(value)}</option>`)}</select></label><label>Verantwortlichkeit<select data-task-filter="assignee"><option value="all">Alle Personen</option>${state.members.map(m=>`<option value="${m.id}" ${taskFilters.assignee===m.id?'selected':''}>${esc(m.name)}</option>`)}</select></label><label>Fälligkeit<select data-task-filter="due">${options('all:Alle Termine|overdue:Überfällig|today:Heute|week:Nächste 7 Tage|none:Ohne Termin',taskFilters.due)}</select></label><label>Status<select data-task-filter="status">${options('all:Alle Status|planned:Geplant|open:Offen|done:Erledigt|refused:Linea wollte nicht',taskFilters.status)}</select></label></div>`;
